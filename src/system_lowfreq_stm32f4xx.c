/**
  ...
  *
  *      - SystemCoreClockLQ variable: Contains the core clock (HCLK), it can be used
  *                                  by the user application to setup the SysTick 
  *                                  timer or configure other parameters.
  *
  *      - SystemCoreClockLQUpdate(): Updates the variable SystemCoreClockLQ and must
...
/**
   * @brief  Update SystemCoreClockLQ variable according to Clock Register Values.
  *         The SystemCoreClockLQ variable contains the core clock (HCLK), it can
  *         be used by the user application to setup the SysTick timer or configure
  *         other parameters.
  *           
  * @note   Each time the core clock (HCLK) changes, this function must be called
  *         to update SystemCoreClockLQ variable value. Otherwise, any configuration
  ...
  *             
  *           - If SYSCLK source is HSI, SystemCoreClockLQ will contain the HSI_VALUE(*)
  *                                              
  *           - If SYSCLK source is HSE, SystemCoreClockLQ will contain the HSE_VALUE(**)
  *                          
  *           - If SYSCLK source is PLL, SystemCoreClockLQ will contain the HSE_VALUE(**) 
  ...
  * @retval None
  */
void SystemCoreClockLQUpdateLQ(void)
{
  ...
  switch (tmp)
  {
    case 0x00:  /* HSI used as system clock source */
      SystemCoreClockLQ = HSI_VALUE;
      break;
    case 0x04:  /* HSE used as system clock source */
      SystemCoreClockLQ = HSE_VALUE;
      break;
  ...
  /* Get HCLK prescaler */
  tmp = AHBPrescTableLQ[((RCC->CFGR & RCC_CFGR_HPRE) >> 4)];
  /* HCLK frequency */
  SystemCoreClockLQ >>= tmp;
}
...
/******************************************************************************/
  __IO uint32_t StartUpCounter = 0, HSEStatus = 0;
    ...
    RCC->PLLCFGR = PLL_M_LQ | (PLL_N_LQ << 6) | (((PLL_P_LQ >> 1) -1) << 16) |
                   (RCC_PLLCFGR_PLLSRC_HSI) | (PLL_Q_LQ << 24);
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    /* Configure the main PLL */
    RCC->PLLCFGR = PLL_M_LQ | (PLL_N_LQ << 6) | (((PLL_P_LQ >> 1) -1) << 16) |
                   (RCC_PLLCFGR_PLLSRC_HSE) | (PLL_Q_LQ << 24);
