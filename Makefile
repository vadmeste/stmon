...
# linker script
LD_SCRIPT = stm32f429i.ld

# output folder (absolute or relative path, leave empty for in-tree compilation)
OUT_DIR = out


# Choose the system clock frequency from the following list : 
# 	SYSCLK_96, SYSCLK_144, SYSCLK_168

SYSCLK = SYSCLK_168
MEMS_CONF = USE_MEMS USE_MEMS_Z USE_MEMS_X USE_MEMS_Y
USB_HS_DMA_ENABLED = USB_OTG_HS_INTERNAL_DMA_ENABLED

# Choose main voltage level between : 
# 	PWR_CR_VOS (Scale 1), PWR_CR_VOS_1 (Scale 2), PWR_CR_VOS_0 (Scale 3) 

VOLTAGE_LEVEL = PWR_CR_VOS

# global definitions for C++, C and ASM (e.g. "symbol_with_value=0xDEAD symbol_without_value")
GLOBAL_DEFS = $(SYSCLK) $(MEMS_CONF) $(USB_HS_DMA_ENABLED) VOS_LEVEL=$(VOLTAGE_LEVEL) \
			  USE_STDPERIPH_DRIVER STM32F429_439xx USE_STM32F429I_DISCO  USE_EMBEDDED_PHY USE_USB_OTG_HS # HSE_VALUE=8000000 # RTC_CLOCK_SOURCE_LSE
...
# flags for C compiler
C_FLAGS = -nostdlib -std=$(C_STD) -g -ggdb3 -fverbose-asm -Wa,-ahlms=$(OUT_DIR_F)$(notdir $(<:.$(C_EXT)=.lst))
